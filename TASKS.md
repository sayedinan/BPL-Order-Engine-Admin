...
- **Sept 1, 2026** — SPEC.md addendum locks the force-change-password flow in-scope: §3.2 (User.mustChangePassword), §3.5 (CHANGE_PASSWORD enum), §4.2 (mustChangePassword in login/me responses; new `POST /api/auth/change-password`), §5.2.1 (ChangePassword page), §5.7 (AuthContext.changePassword), §9 clarified that the deferred item is the *unauthenticated* password-reset/recovery flow. #14a, #14b, #24a are now unblocked at the SPEC layer. `TASKS-decomposed.md` updated: #15 🔒 count 6 → 7 (15.4, 15.6, 15.7, 15.12, 15.13 added); #19.3 split into 19.3a/19.3b; #16.5, #22.7–#22.9 "done when" rewritten to paste-able assertions.
